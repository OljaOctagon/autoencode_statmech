...
def append_w4w6_no_average(npz_path, traj_path, batch_size, overwrite=False):
    data = load_npz_dict(npz_path)
    if "w4w6_no_average" in data and not overwrite:
        raise ValueError(
            f"{npz_path} already contains 'w4w6_no_average'. "
    ...
    out = np.empty((picked_ids.shape[0], 2), dtype=np.float32)
    ...
    for batch_start, Config, Box, _ in tqdm(
        ...
        total=total_batches,
        desc="Appending w4w6_no_average",
    ):
        ...
        for local_t in range(batch_len):
            ...
            if not np.any(sample_mask):
                continue
            out[sample_mask] = w4w6_no_average[
                local_t,
                picked_ids[sample_mask],
                :,
            ]

    data["w4w6_no_average"] = out
