...
def _compute_ql_trajectory(
    ...
    """
    Compute averaged q_l (Lechner–Dellago) for all frames up to t_max.
    ...
    # Pre-allocate: (T_used, Natoms, 2)
    q_traj_all = np.empty((t_max, Natoms, len(l_list)), dtype=np.float32)
    ...
    for t in range(t_max):
        ...
        # Compute averaged q_l for this frame
        steinhardt.compute(
            system=(box, positions),
            neighbors={"num_neighbors": num_neighbors},
        )
        ...
        )  # shape (Natoms, 2): [q_1 to q_20]

    return q_traj_all  # shape (t_max,Natoms,2)
